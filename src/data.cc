...
// open a raster file
GDALDatasetH raster_open(char * pszFilename, double *georef, char *prj, double *nodata, int *x, int *y)
{
	...
	if (GDALGetGeoTransform( hDataset, georef) == CE_Failure) {
		fprintf(stderr, "Error fetch geo transformation info on raster %s\n", pszFilename);
		exit(1);
	}
...
int raster_read(GDALDatasetH hDataset, float *buf, int offsetx, int offsety, int sizex, int sizey)
{
	...
	GDALRasterIO( hBand, GF_Read, offsetx, offsety, sizex, sizey, pafScanline, sizex, sizey, GDT_Float32, 0, 0 );
	int i;
	for (i=0; i<nSize; i++) {
		buf[i] = pafScanline[i];
	}
...
int raster_write(GDALDatasetH hDstDS, float *buf, int offsetx, int offsety, int sizex, int sizey)
{
	...
	GDALRasterIO( hBand, GF_Write, offsetx, offsety, sizex, sizey, buf, sizex, sizey, GDT_Float32, 0, 0 );
	return 1;
}
#endif
